...
def train(model, data):
    ...
    # callbacks
    log = callbacks.CSVLogger(save_dir + '/Digitslog.csv')
    tb = callbacks.TensorBoard(log_dir=save_dir + '/DigitsTensorboard-logs',
                               batch_size=batch_size, histogram_freq=int(debug))
    checkpoint = callbacks.ModelCheckpoint(save_dir + 'Digitsweights-{epoch:02d}.h5', monitor='val_acc',
    ...
    model.save_weights(save_dir + 'Digits_Final_Trained_Model.h5')
    print('Trained model saved to' + save_dir + '/Digits_Final_Trained_Model.h5')

    from utils import plot_log
    plot_log(save_dir + '/Digitslog.csv', show=True)
    return model
def test(model, data):
    x_test, y_test = data
    ...
    image = img * 255
    Image.fromarray(image.astype(np.uint8)).save(save_dir + "/DigitsReal_and_recon.png")
    print('Reconstructed images are saved to %s/DigitsReal_and_recon.png' % save_dir)
    print('-' * 30 + 'End: test' + '-' * 30)
    plt.imshow(plt.imread(save_dir+ "/DigitsReal_and_recon.png"))
    plt.show()
def load_mnist():
    """
    ...
    """
    import pandas as pd
    train = pd.read_csv('../input/emnist-digits-train.csv', header=None)
    test = pd.read_csv('../input/emnist-digits-test.csv', header=None)
